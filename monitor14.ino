...
// ==========================================
// БАГАТОЗАДАЧНІСТЬ: Modbus на ядрі 0
// ==========================================
SemaphoreHandle_t dataMutex = nullptr;
TaskHandle_t modbusTaskHandle = nullptr;
volatile bool webUpdateActive = false;
volatile bool pendingCounterReset = false; // команда скидання лічильників
...
struct InverterData {
  ...
  float selfConsumptionFiltered = 0.0f;

  float battPowerSigned = 0.0f;
  float battPowerMeasured = 0.0f;
  float battPowerFiltered = 0.0f;
  float battAmpDisplay = 0.0f;
  int battWattDisplay = 0;

  int gridPowerShow = 0;
  int systemBalanceShow = 0;

  int offgridEnable = 0;
  int energyMode = 0;
  int chargerPriority = 0;

  const char* stateStr = "OFF";
  const char* workModeShort = "OFF";

  const char* batModeUi = "IDLE";
  const char* batClassUi = "blue";
  bool batRegsValid = false;

  String machineType;
  String machinePower;

  uint16_t err1 = 0, err2 = 0, err3 = 0;
  uint16_t warn1 = 0, warn2 = 0;
  uint16_t chgErr = 0, chgWarn = 0;
};
...
String fRelay(uint16_t raw) {
  if (raw == 0xFFFF) return "--";
  return (raw == 0) ? "Disconnect" : "Connect";
}
// За протоколом MUST: старше слово = 1000 kWh, молодше = 0.1 kWh
...
const char* workShort(int ws) {
  switch (ws) {
    case 0: return "ON";     case 1: return "TEST";  case 2: return "BATT";
    case 3: return "TIE";    case 4: return "BYPASS";case 5: return "STOP";
    case 6: return "CHRG";   default: return "--";
  }
}
...
String decodeErrors(uint16_t e1, uint16_t e2, uint16_t e3) {
  ...
  String errs = "";
  const char* err1Bits[] = {
    "Fan locked", "Transformer overtemp", "Batt V high", "Batt V low",
  ...
  const char* err2Bits[] = {
    "Radiator overtemp", "Charger batt class err", "Charger I sensor", "Charger I uncontrolled",
  ...
  for (int i = 0; i < 16; i++) {
    if (e1 & (1 << i)) { if (errs.length() > 0) errs += ", "; errs += err1Bits[i]; }
    if (e2 & (1 << i)) { if (errs.length() > 0) errs += ", "; errs += err2Bits[i]; }
  }
...
String decodeWarnings(uint16_t w1, uint16_t w2) {
  ...
  const char* warn1Bits[] = {
    "Fan locked on", "Fan2 locked on", "Batt overcharged", "Low battery",
    "Overload", "Power derating", "Charger low batt", "Charger high PV",
    "Charger overload", "Charger overtemp", "Charger comm error"
  };
  String warns = "";
  for (int i = 0; i < 11; i++) {
    if (w1 & (1 << i)) { if (warns.length() > 0) warns += ", "; warns += warn1Bits[i]; }
  }
...
String decodeChgErrors(uint16_t e) {
  ...
  const char* chgErrBits[] = {
    "Hardware protect", "Overcurrent", "I sensor error", "Overtemp",
    "PV V high", "PV V low", "Batt V high", "Batt V low",
    "I uncontrolled", "Parameter error"
  };
  String errs = "";
  for (int i = 0; i < 10; i++) {
    if (e & (1 << i)) { if (errs.length() > 0) errs += ", "; errs += chgErrBits[i]; }
  }
  return errs.length() > 0 ? errs : "No errors";
}
...
String battTypeStr(uint16_t r) {
  if (r == 0xFFFF) return "--";
  switch (r) {
    case 0: return "User defined";
    case 1: return "Use defined";
    case 2: return "Lithium";
    case 3: return "Sealed lead";
    case 4: return "AGM";
    case 5: return "GEL";
    case 6: return "Flooded";
...
String buildRecommendations() {
  String r = "";
  if (inv.err1 || inv.err2 || inv.err3)
    r += "⛔ Помилка інвертора: " + decodeErrors(inv.err1, inv.err2, inv.err3) + "\n";
  if (inv.chgErr)
    r += "⛔ Помилка зарядного пристрою: " + decodeChgErrors(inv.chgErr) + "\n";
  if (inv.warn1 || inv.warn2)
    r += "⚠️ Попередження: " + decodeWarnings(inv.warn1, inv.warn2) + "\n";
  if (inv.tempAC > 60)
    r += "🌡️ Перегрів AC радіатора: " + String(inv.tempAC) + "C — перевірте охолодження\n";
  if (inv.tempDC > 60)
    r += "🌡️ Перегрів DC радіатора: " + String(inv.tempDC) + "C — зменште навантаження\n";
  ...
  if (inv.workState == 4)
    r += "🔌 Інвертор у BYPASS — навантаження живиться безпосередньо, батарея не використовується\n";
...
// ==========================================
// РОЗРАХУНОК ТА АНАЛІЗ БАТАРЕЇ ТА БАЛАНСУ
// ==========================================
...
// Універсальна перевірка напруги АКБ: 12V / 24V / 48V системи
inline bool battVoltageOk(float v) {
  return (v >= 9.0f && v <= 17.0f) ||   // 12V
         (v >= 18.0f && v <= 35.0f) ||  // 24V
         (v >= 36.0f && v <= 70.0f);    // 48V
}
...
// Чи живі регістри батареї (25273 = W, 25274 = A)
static bool r73Sane() {
  return !isBatRegDead(inv.inv[72]) && std::abs((int16_t)inv.inv[72]) <= (int)maxBatW();
}
static bool r74Sane() {
  return !isBatRegDead(inv.inv[73]) && std::fabs(ampsFrom74((int16_t)inv.inv[73])) <= maxBatA();
}
...
void updateBatteryTruth() {
  ...
  if (r73Sane() && std::abs((int16_t)inv.inv[72]) >= 15) {
    have = true;
    batPower = (float)std::abs((int16_t)inv.inv[72]);
    ...
    have = true;
    ampTruth = ampsFrom74((int16_t)inv.inv[73]);
  ...
  if (!have && vOk && (inv.workState == 2 || inv.workState == 3)) {
    ...
    if (est >  pMax) est =  pMax;
    if (est < -pMax) est = -pMax;
    if (std::fabs(est) >= kBatThrW) {
      have = true;
      batPower = est;
      ampTruth = est / vBat;
    }
  ...
  if (!have) { inv.battPowerFiltered = 0.0f; batPower = 0.0f; ampTruth = 0.0f; }
    ...
    inv.battPowerFiltered = inv.battPowerFiltered * 0.5f + batPower * 0.5f;
  }
  ...
  if (batPower > kBatThrW) { inv.batModeUi = "CHARGE"; inv.batClassUi = "green"; }
    ...
    inv.batModeUi = "IDLE"; inv.batClassUi = "blue";
...
void updateBatteryMetrics() {
  ...
  const bool gridOn = (snap.vGrid > 80.0f) || gridImportW > 20.0f;
  ...
  // Реконструкція імпорту з мережі ЛИШЕ у режимі GRID CHRG (workState 6)
  ...
  if (gridOn && snap.workState == 6 && gridTotalIn > 10.0f)
    inv.gridPowerShow = -(int)roundf(gridTotalIn);
  ...
  float netFromGrid = gridOn ? ((float)snap.loadW + chargeW - dischargeW - (float)snap.solarW) : fmaxf(0.0f, (float)snap.loadW - (float)snap.solarW);
  inv.systemBalanceShow = (int)roundf(-netFromGrid);
  inv.pNet = inv.systemBalanceShow;
}

// ==========================================
// ЗАДАЧА ОПИТУВАННЯ MODBUS (ЯДРО 0)
// ==========================================
void pollModbusOnce() {
  bool ok = true;
  ...
  inv.vOut = inv.inv[5] / 10.0f;
  inv.vGrid = inv.inv[6] / 10.0f;
  inv.vBus = inv.inv[7] / 10.0f;
  inv.gridPower = (int16_t)inv.inv[13];
  inv.pLoad = inv.inv[14];
  inv.loadPercent = constrain(inv.inv[15], 0, 100);
  inv.tempAC = (int16_t)inv.inv[32];
  inv.tempDC = (int16_t)inv.inv[34];
  inv.vPV = inv.pv[4] / 10.0f;
  inv.solarPowerRaw = inv.pv[7];
  inv.effectiveSolarW = (inv.pv[1] == 1 || inv.pv[1] == 2) ? inv.solarPowerRaw : 0;

  // PV ENERGY: старше слово = 1000 kWh, молодше = 0.1 kWh
  inv.pvEnergy = (float)inv.pv[16] * 1000.0f + (float)inv.pv[17] / 10.0f;

  inv.offgridEnable = inv.ctrl[0];
  inv.energyMode = inv.ctrl[8];
  inv.chargerPriority = inv.ctrl[42];
  ...
  snap.valid = true;
  snap.workState = inv.workState;
  snap.vGrid = inv.vGrid;
  snap.gridPowerRaw = inv.gridPower;
  snap.loadW = inv.pLoad;
  snap.solarW = inv.effectiveSolarW;
...
void modbusTask(void* pvParameters) {
  (void)pvParameters;
  for (;;) {
    // Скидання лічильників за командою з веб-інтерфейсу (рег. 20213 та 10112)
    if (pendingCounterReset) {
      pendingCounterReset = false;
      bool ok1 = (node.writeSingleRegister(20213, 1) == node.ku8MBSuccess);
      delay(100);
...
.box{background:#1c1c1c;border-radius:10px;padding:12px;text-align:center}
.label{color:#888;font-size:11px}
.value{font-size:18px;margin-top:8px;font-weight:bold}
...
<div class="grid">
  ...
  <div class="box"><div class="label">INV LOSS</div><div class="value yellow" id="invLoss">0 W</div></div>
...
function doUpdate(){
  ...
  xhr.upload.onprogress=function(e){
    if(e.lengthComputable){
      ...
      s("updStatus","Завантаження: "+p+"%");
    }
  };
  xhr.onload=function(){
    if(xhr.status==200 && xhr.responseText=="OK"){
      document.getElementById("prog").style.width="100%";
      s("updStatus","✅ Прошивку записано! Перезавантаження...");
      setTimeout(function(){location.reload();},10000);
    }else{
      s("updStatus","❌ Помилка оновлення: "+xhr.responseText);
    }
  };
...
async function update(){
  try{
    ...
    s("tempAC",d.tempAC+"C"); s("solar",d.pvvolt+"V | "+d.solar+"W");
    s("grid",d.grid+"V | "+d.gridPowerShow+"W"); s("load",d.loadVolt+"V | "+d.loadPower+"W");
    s("invLoss",d.invLoss+" W"); s("pvEnergy",d.pvEnergy+" kWh");
    let b=document.getElementById("bat"); b.className="value "+d.batClass;
    b.innerText=d.batMode+" | "+d.batteryVolt+" V | "+d.batPowerDisplay+" W | "+d.battAmp+" A"+d.batSuffix;
    setBalance(d.net,d.workMode||d.state,d.loadPercent);
    s("offgridEnable",d.offgridEnable); s("energyMode",d.energyMode); s("chargerPriority",d.chargerPriority);
    s("recs",d.recs);
...
void setupWebServer() {
  ...
  // Скидання накопичувальних лічильників інвертора (виконується задачею Modbus)
  ...
  server.on("/api", HTTP_GET, [](AsyncWebServerRequest *req){
    bool locked = (dataMutex != nullptr) &&
                  (xSemaphoreTake(dataMutex, pdMS_TO_TICKS(200)) == pdTRUE);

    float loadVoltage = (inv.workState == 2 || inv.workState == 3) ? inv.vOut :
                        ((inv.workState == 4 || inv.workState == 6) ? inv.vGrid : 0.0f);
    ...
    String recs = buildRecommendations();
    recs.replace("\n", "\\n");
    ...
    String bs_chgtime = "--";
    if (inv.ctrl[9] == 1 && inv.ctrl[36] != 0xFFFF) {
      char buf[32];
      snprintf(buf, sizeof(buf), "%02d:%02d - %02d:%02d",
               (int)inv.ctrl[36], (int)inv.ctrl[37], (int)inv.ctrl[38], (int)inv.ctrl[39]);
      bs_chgtime = String(buf);
    }
    
    String bs_distime = "--";
    if (inv.ctrl[40] != 0xFFFF) {
      char buf[32];
      snprintf(buf, sizeof(buf), "%02d:%02d - %02d:%02d",
               (int)inv.ctrl[40], (int)inv.ctrl[41], (int)inv.ctrl[42], (int)inv.ctrl[43]);
      bs_distime = String(buf);
    }
    ...
    String ia_rated   = (inv.inv[76] != 0xFFFF && inv.inv[76] != 0) ? String(inv.inv[76]) + " VA"
                        : (inv.machinePower + " VA");
...
void loop() {
  ...
  if (needRestart && (currentMs - restartRequestedMs >= RESTART_DELAY_MS)) {
    ESP.restart();
  }
  ...
  if (WiFi.status() != WL_CONNECTED) {
    if (currentMs - lastWifiReconnectMs >= WIFI_RECONNECT_INTERVAL_MS) {
      lastWifiReconnectMs = currentMs;
      WiFi.reconnect();
    }
  }
